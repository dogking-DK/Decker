...
namespace {
                               ...
                               const vk::Extent2D& current_extent)
    {
        ...
        {
            fmt::print("(Swapchain) Image extent ({}, {}) not supported. Selecting ({}, {}).",
                                           ...
                                           const std::vector<vk::PresentModeKHR>& present_mode_priority_list)
    {
        ...
        if (present_mode_it == available_present_modes.end())
        {
            ...
            const vk::PresentModeKHR chosen_present_mode = (chosen_present_mode_it != present_mode_priority_list.
                                                            end())
                                                               ? *chosen_present_mode_it
                                                               : vk::PresentModeKHR::eFifo;

            fmt::print("(Swapchain) Present mode '{}' not supported. Selecting '{}'.",
                       to_string(request_present_mode), to_string(chosen_present_mode));
            return chosen_present_mode;
        }
        fmt::print("(Swapchain) Present mode selected: {}", to_string(request_present_mode));
                                               ...
                                               surface_format_priority_list)
    {
        ...
        if (surface_format_it == available_surface_formats.end())
        {
            ...
            fmt::print("(Swapchain) Surface format ({}) not supported. Selecting ({}).",
                       to_string(requested_surface_format.format) + ", " + to_string(
                           requested_surface_format.colorSpace),
                       to_string(chosen_surface_format.format) + ", " + to_string(
                           chosen_surface_format.colorSpace));
            return chosen_surface_format;
        }
        fmt::print("(Swapchain) Surface format selected: {}",
                   to_string(requested_surface_format.format) + ", " + to_string(
                       requested_surface_format.colorSpace));
                                                     ...
                                                     vk::SurfaceTransformFlagBitsKHR current_transform)
    {
        ...
        fmt::print("(Swapchain) Surface transform '{}' not supported. Selecting '{}'.",
                                                         ...
                                                         vk::CompositeAlphaFlagsKHR    supported_composite_alpha)
    {
        ...
        fmt::print("(Swapchain) Composite alpha '{}' not supported. Selecting '{}.",
        ...
        vk::FormatFeatureFlags                  supported_features)
    {
        ...
        for (auto flag : requested_image_usage_flags)
        {
            ...
            {
                fmt::print("(Swapchain) Image usage ({}) requested but not supported.", to_string(flag));
            }
        }
        ...
        fmt::print("(Swapchain) Image usage flags: {}", usage_list);
