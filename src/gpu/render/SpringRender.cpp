...
namespace dk {
    ...
    SpringRenderer::SpringRenderer(dk::vkcore::VulkanContext* context)
        : _context(context)
    {
    }
    ...
    {
        _max_particle_count = 1000000;
        _max_spring_count = 2000000;
    ...
    void SpringRenderer::createPipeline(vk::Format color_format, vk::Format depth_format)
    {
        // 1. 创建布局
        {
            ...
            layout_builder
                .addBinding(0, vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eMeshEXT) // binding=0: Camera UBO
                .addBinding(1, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eMeshEXT) // binding=1: Particle Data SSBO
                .addBinding(2, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eMeshEXT); // binding=2: Spring Index SSBO
        ...
        try {
            ...
            // **你需要创建这两个新的着色器文件**
            fs::path target_file_mesh = fs::absolute(current_dir / "../../assets/shaders/spv/spring.mesh.spv");
            fs::path target_file_frag = fs::absolute(current_dir / "../../assets/shaders/spv/spring.frag.spv");

            mesh_module = std::make_unique<dk::vkcore::ShaderModule>(_context, dk::vkcore::loadSpirvFromFile(target_file_mesh));
            frag_module = std::make_unique<dk::vkcore::ShaderModule>(_context, dk::vkcore::loadSpirvFromFile(target_file_frag));
        }
    ...
    void SpringRenderer::draw(dk::vkcore::CommandBuffer& cmd, const CameraData& camera_data, VkRenderingInfo& render_info)
    {
